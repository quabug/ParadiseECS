...
public class ComponentGenerator : IIncrementalGenerator
{
    ...
    private static GeneratorConfig ExtractConfig(Compilation compilation, AnalyzerConfigOptionsProvider options)
    {
        ...
        var enableTags = compilation.Assembly.GetAttributes()
            .Any(a => a.AttributeClass?.ToDisplayString() == EnableTagsAttributeFullName);

        return new GeneratorConfig(rootNamespace, maxComponentTypeId, suppressGlobalUsings, enableTags);
    }
        ...
        GeneratorConfig config)
    {
        ...
        var hasEntityTags = components.Any(c => c.TypeName == "EntityTags");
        var tagsEffectivelyEnabled = config.EnableTags && validTags.Count > 0 && hasEntityTags;
        ...
        if (validComponents.Count > 0)
        {
            ...
            foreach (var component in validComponents)
                GeneratePartialStruct(context, component, tagMaskType, "");
    ...
    private static void GeneratePartialStruct(SourceProductionContext context, TypeInfo info, string tagMaskType, string filePrefix)
    {
        ...
        {
            var isEntityTags = info.TypeName == "EntityTags";
            ...
            sb.AppendLine($"{indent}partial struct {info.TypeName} : {interfaces}");
            sb.AppendLine($"{indent}{{");
