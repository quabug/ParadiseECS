...
public sealed class ArchetypeStoreTests : IDisposable
{
    ...
    public void Dispose()
    {
        ...
        {
            ImmutableArchetypeLayout<Bit64, ComponentRegistry, DefaultWorldConfig>.Free(NativeMemoryAllocator.Shared, layoutData);
        }
        _chunkManager?.Dispose();
    }

    private Archetype<Bit64, ComponentRegistry, DefaultWorldConfig> CreateStore(params ComponentTypeInfo[] components)
    {
        ...
        var layoutData = ImmutableArchetypeLayout<Bit64, ComponentRegistry, DefaultWorldConfig>.Create(NativeMemoryAllocator.Shared, mask);
        _layoutDataList.Add(layoutData);
        return new Archetype<Bit64, ComponentRegistry, DefaultWorldConfig>(_layoutDataList.Count - 1, layoutData, _chunkManager);
    }
