...
public sealed class WorldTests
{
    ...
    public async Task Constructor_CreatesValidWorld()
    {
        ...
        using var world = new World<SmallBitSet<ulong>, ComponentRegistry, DefaultConfig>(config, sharedMetadata, chunkManager);
    ...
    public async Task Constructor_NullChunkManager_Throws()
    {
        ...
        await Assert.That(() => new World<SmallBitSet<ulong>, ComponentRegistry, DefaultConfig>(config, sharedMetadata, null!))
    ...
    public async Task Constructor_ZeroOrNegativeCapacity_ThrowsArgumentOutOfRangeException()
    {
        ...
        using var sharedMetadata1 = new SharedArchetypeMetadata<SmallBitSet<ulong>, ComponentRegistry, DefaultConfig>(config0);
        await Assert.That(() =>
        {
            using var world = new World<SmallBitSet<ulong>, ComponentRegistry, DefaultConfig>(config0, sharedMetadata1, chunkManager1);
        }).Throws<ArgumentOutOfRangeException>();

        var configNeg = new DefaultConfig { DefaultEntityCapacity = -1 };
        using var chunkManager2 = ChunkManager.Create(configNeg);
        using var sharedMetadata2 = new SharedArchetypeMetadata<SmallBitSet<ulong>, ComponentRegistry, DefaultConfig>(configNeg);
        await Assert.That(() =>
        {
            using var world = new World<SmallBitSet<ulong>, ComponentRegistry, DefaultConfig>(configNeg, sharedMetadata2, chunkManager2);
        }).Throws<ArgumentOutOfRangeException>();
    }
    ...
    public async Task Dispose_PreventsNewOperations()
    {
        ...
        using var chunkManager = ChunkManager.Create(config);
        using var sharedMetadata = new SharedArchetypeMetadata<SmallBitSet<ulong>, ComponentRegistry, DefaultConfig>(config);
        var world = new World<SmallBitSet<ulong>, ComponentRegistry, DefaultConfig>(config, sharedMetadata, chunkManager);
        world.Dispose();
...
public sealed class WorldSharedMetadataTests
{
    ...
    {
        var config = new DefaultConfig();
        using var customMetadata = new SharedArchetypeMetadata<SmallBitSet<ulong>, ComponentRegistry, DefaultConfig>(config);
        using var chunkManager = ChunkManager.Create(config);
        using var world = new World<SmallBitSet<ulong>, ComponentRegistry, DefaultConfig>(config, customMetadata, chunkManager);
    ...
    public async Task Constructor_NullSharedMetadata_Throws()
    {
        ...
        await Assert.That(() => new World<SmallBitSet<ulong>, ComponentRegistry, DefaultConfig>(config, null!, chunkManager))
            .ThrowsExactly<ArgumentNullException>();
    }
    ...
    public async Task MultipleWorlds_SameSharedMetadata_ShareMetadata()
    {
        ...
        using var world1 = new World<SmallBitSet<ulong>, ComponentRegistry, DefaultConfig>(config, sharedMetadata, chunkManager1);
        using var world2 = new World<SmallBitSet<ulong>, ComponentRegistry, DefaultConfig>(config, sharedMetadata, chunkManager2);
    ...
    {
        var config = new DefaultConfig();
        using var metadata1 = new SharedArchetypeMetadata<SmallBitSet<ulong>, ComponentRegistry, DefaultConfig>(config);
        using var metadata2 = new SharedArchetypeMetadata<SmallBitSet<ulong>, ComponentRegistry, DefaultConfig>(config);
        using var chunkManager1 = ChunkManager.Create(config);
        using var chunkManager2 = ChunkManager.Create(config);
        using var world1 = new World<SmallBitSet<ulong>, ComponentRegistry, DefaultConfig>(config, metadata1, chunkManager1);
        using var world2 = new World<SmallBitSet<ulong>, ComponentRegistry, DefaultConfig>(config, metadata2, chunkManager2);
    ...
    {
        var config = new DefaultConfig();
        using var metadata = new SharedArchetypeMetadata<SmallBitSet<ulong>, ComponentRegistry, DefaultConfig>(config);
        using var chunkManager = ChunkManager.Create(config);

        var world = new World<SmallBitSet<ulong>, ComponentRegistry, DefaultConfig>(config, metadata, chunkManager);
        world.Dispose();
