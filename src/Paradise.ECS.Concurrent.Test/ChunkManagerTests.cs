...
public class ChunkManagerExceptionTests : IDisposable
{
    ...
    {
        var manager = new ChunkManager<DefaultWorldConfig>(initialCapacity: 4);
        manager.Dispose();
    ...
    {
        using var manager = new ChunkManager<DefaultWorldConfig>(initialCapacity: 4);
...
public class ChunkManagerConstructorTests
{
    ...
    {
        await Assert.That(() => new ChunkManager<DefaultWorldConfig>(null!, 16)).Throws<ArgumentNullException>();
    }

    [Test]
    public async Task Constructor_WithZeroInitialCapacity_UsesMinimumCapacity()
    {
        using var manager = new ChunkManager<DefaultWorldConfig>(initialCapacity: 0);
        var handle = manager.Allocate();
        await Assert.That(handle.IsValid).IsTrue();
    }
    ...
    public async Task Constructor_WithLargeInitialCapacity_CapsToMaxBlocks()
    {
        ...
        using var manager = new ChunkManager<DefaultWorldConfig>(initialCapacity: 2_000_000);
        var handle = manager.Allocate();
        await Assert.That(handle.IsValid).IsTrue();
    }
}
